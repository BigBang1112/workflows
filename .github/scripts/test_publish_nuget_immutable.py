...
import publish_nuget_immutable as target
...
class PublishNugetTests(unittest.TestCase):
    def test_prepare_matches_packed_identity_to_csproj_release_notes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(str(Path("Src/PackageX/PackageX.csproj")), manifest[0]["project"])
    ...
    def test_release_notes_use_only_uploaded_packages(self):
        environment = {
            "MAIN_PROJECT": "Package.X",
    ...
    def test_release_attaches_only_uploaded_package_assets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIn(str(Path("packages/New.1.0.0.nupkg")), command)
                self.assertNotIn(str(Path("packages/Duplicate.1.0.0.nupkg")), command)
                self.assertIn("--verify-tag", command)
            finally:
                os.chdir(old_directory)

    def test_configured_feeds_honor_selected_destinations(self):
        environment = {
            "PUSH_TO_NUGET": "false",
            "PUSH_TO_GITHUB": "false",
            "PUSH_TO_CUSTOM_FEEDS": "true",
            "CUSTOM_FEED_URLS": "https://one.example/v3/index.json\nhttps://two.example/v3/index.json",
            "CUSTOM_FEED_API_KEYS": "one-key\ntwo-key",
        }
        with patch.dict(os.environ, environment, clear=True):
            feeds = target.configured_feeds()
        self.assertEqual([
            ("Custom feed 1", "https://one.example/v3/index.json", "one-key"),
            ("Custom feed 2", "https://two.example/v3/index.json", "two-key"),
        ], feeds)
    ...
    def test_discord_message_formats_packages_and_links(self):
        ...
        environment = {
            "DISCORD_TOP_LINES": "Introduction",
            "DISCORD_BOTTOM_LINES": "Thanks for using Package.X!",
        }
